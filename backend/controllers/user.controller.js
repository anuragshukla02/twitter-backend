...
export const updateUser = async(req,res)=>{
    const {username,email,fullName,bio,link,currentPassword,newPassword} = req.body
    let {profileImg,coverImg} = req.body

    try {
       let user = await User.findById(req.user._id) 
        if(!currentPassword&&newPassword || currentPassword&& !newPassword){
            return res.status(400).json({error:"send both the password:current and new"})
        }

        if(currentPassword&&newPassword){
            const isMatch = await bcrypt.compare(currentPassword,user.password)
            if(!isMatch){
                return res.status(400).json({error:"please send the correct current password"})
            }
            if(newPassword.length<6){
                return res.status(400).json({error:"new password must be 6 character long"})
            }
